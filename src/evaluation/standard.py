...
def get_directory_dict(scenario_parameters):
    ...
    day_dir = scenario_parameters[G_DAY_DIR]
    ...
    dirs[G_DIR_DEMAND] = os.path.join(dirs[G_DIR_DATA], "demand", demand_name, "matched", network_name, day_dir)
...
def standard_evaluation(output_dir, evaluation_start_time = None, evaluation_end_time = None, print_comments=False, dir_names_in = {}):
    ...
    for op_id, op_users in user_stats.groupby(G_RQ_OP_ID):
        ...
        op_90perquant_morning_wait_time = np.nan

        op_afternoon_avg_wait_time = np.nan
        op_med_afternoon_wait_time = np.nan
        op_90perquant_afternoon_wait_time = np.nan

        op_evening_avg_wait_time = np.nan
        op_med_evening_wait_time = np.nan
        op_90perquant_evening_wait_time = np.nan
        ...
        if op_id >= 0:  #AMoD
            ...
            if G_RQ_PU in op_users.columns and G_RQ_TIME in op_users.columns:
                ...
                if scenario_parameters[G_MORNING_HR_START] and scenario_parameters[G_MORNING_HR_END]:
                    mask_morning = (op_users[G_RQ_TIME] >= scenario_parameters[G_MORNING_HR_START]) & (
                            op_users[G_RQ_TIME] <= scenario_parameters[G_MORNING_HR_END])
                    morning_wait_times = op_users.loc[mask_morning, "wait time"]
                    op_morning_avg_wait_time = morning_wait_times.mean()
                    op_med_morning_wait_time = morning_wait_times.median()
                    op_90perquant_morning_wait_time = morning_wait_times.quantile(q=0.9)

                if scenario_parameters[G_AFTERNOON_HR_START] and scenario_parameters[G_AFTERNOON_HR_END]:
                    mask_afternoon = (op_users[G_RQ_TIME] >= scenario_parameters[G_AFTERNOON_HR_START]) & \
                                     (op_users[G_RQ_TIME] <= scenario_parameters[G_AFTERNOON_HR_END])
                    afternoon_wait_times = op_users.loc[mask_afternoon, "wait time"]
                    op_afternoon_avg_wait_time = afternoon_wait_times.mean()
                    op_med_afternoon_wait_time = afternoon_wait_times.median()
                    op_90perquant_afternoon_wait_time = afternoon_wait_times.quantile(q=0.9)

                if scenario_parameters[G_EVENING_HR_START] and scenario_parameters[G_EVENING_HR_END]:
                    mask_evening = (op_users[G_RQ_TIME] >= scenario_parameters[G_EVENING_HR_START]) & \
                                   (op_users[G_RQ_TIME] <= scenario_parameters[G_EVENING_HR_END])
                    evening_wait_times = op_users.loc[mask_evening, "wait time"]
                    op_evening_avg_wait_time = evening_wait_times.mean()
        ...
        result_dict["waiting time for morning (90% quantile)"] = op_90perquant_morning_wait_time

        result_dict["waiting time for afternoon"] = op_afternoon_avg_wait_time
        result_dict["waiting time for afternoon (median)"] = op_med_afternoon_wait_time
        result_dict["waiting time for afternoon (90% quantile)"] = op_90perquant_afternoon_wait_time

        result_dict["waiting time for evening"] = op_evening_avg_wait_time
        result_dict["waiting time for evening (median)"] = op_med_evening_wait_time
        result_dict["waiting time for evening (90% quantile)"] = op_90perquant_evening_wait_time

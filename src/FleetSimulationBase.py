...
INPUT_PARAMETERS_FleetSimulationBase = {
    ...
    "input_parameters_optional": [
        G_SIM_TIME_STEP, G_NR_CH_OPERATORS, G_SIM_REALTIME_PLOT_FLAG, "log_level", G_SIM_ROUTE_OUT_FLAG,
        G_SIM_REPLAY_FLAG, G_INIT_STATE_SCENARIO,
...
class FleetSimulationBase:
    def __init__(self, scenario_parameters: dict):
        ...
        logging.LoggerAdapter.verbose = lambda inst, msg, *args, **kwargs: inst.log(logging.VERBOSE, msg, *args,
                                                                                    **kwargs)
        ...
        if self.dir_names.get(G_DIR_ZONES, None) is not None:
            if self.scenario_parameters.get(G_FC_TYPE) and self.scenario_parameters[G_FC_TYPE] == "perfect":
                ...
                self.zones = PerfectForecastZoneSystem(self.dir_names[G_DIR_ZONES], self.scenario_parameters,
                                                       self.dir_names)
            else:
        ...
        if pt_type is None or self.gtfs_data_dir is None:
            ...
            self.pt = pt_module.PublicTransportWithCrowding(self.gtfs_data_dir, self.pt_stat_f,
                                                            self.scenario_parameters,
                                                            self.routing_engine, self.zones)
        else:
        ...
        self.vehicle_update_order: tp.Dict[tp.Tuple[int, int], int] = {vid: 1 for vid in
                                                                       self.sim_vehicles.keys()}  # value defines the order in whichvehicles are updated (i.e. charging first)
        ...
        if self.scenario_parameters[G_SIM_ENV] != "MobiTopp":
            ...
            if self.scenario_parameters.get(G_PA_RQ_FILE) is not None:
                self.demand.load_parcel_demand_file(self.scenario_parameters[G_SIM_START_TIME],
                                                    self.scenario_parameters[G_SIM_END_TIME],
                                                    self.dir_names[G_DIR_PARCEL_DEMAND],
                                                    self.scenario_parameters[G_PA_RQ_FILE],
                                                    self.scenario_parameters[G_RANDOM_SEED],
        ...
        if self.dir_names.get(G_DIR_INFRA):
            ...
            for op_id, op_dict in enumerate(self.list_op_dicts):
                ...
                if depot_f_name is not None:
                    ...
                    op_charge = OperatorChargingAndDepotInfrastructure(op_id, depot_f, op_dict,
                ...
                for ch_op_id, ch_op_dict in enumerate(self.list_ch_op_dicts):
                    ...
                    if pub_cs_f_name is None:
                        raise EnvironmentError(
                            "Public charging stations file not given as input! parameter {} required!".format(
                                G_CH_OP_F))
                    ...
                    ch_op = PublicChargingInfrastructureOperator(ch_op_id, pub_cs_f, ch_op_dict,
                                                                 self.scenario_parameters, self.dir_names,
                                                                 self.routing_engine,
                                                                 initial_charging_events_f=initial_ch_events_f)
        ...
        for op_id in range(self.n_op):
            ...
            if not operator_module_name == "LinebasedFleetControl":
                ...
                for veh_type, nr_veh in fleet_composition_dict.items():
                    for _ in range(nr_veh):
                        ...
                        tmp_veh_obj = SimulationVehicle(op_id, vid, self.dir_names[G_DIR_VEH], veh_type,
                                                        ...
                                                        replay_flag, self.scenario_parameters)
                ...
                self.operators.append(
                    OpClass(op_id, operator_attributes, list_vehicles, self.routing_engine, self.zones,
                            self.scenario_parameters, self.dir_names,
                ...
                OpClass = LinebasedFleetControl(op_id, self.gtfs_data_dir, self.routing_engine, self.zones,
                                                self.scenario_parameters, self.dir_names,
                                                self.charging_operator_dict["op"].get(op_id, None),
                                                list(self.charging_operator_dict["pub"].values()))
        ...
        if self.scenario_parameters.get(G_INIT_STATE_SCENARIO):
            ...
            if not os.path.isfile(init_state_f):
                raise FileNotFoundError(
                    f"init state variable {G_INIT_STATE_SCENARIO} given but file {init_state_f} not found!")
        ...
        if len(set_unassigned_vid) > 0:
            ...
            for op_id in range(self.n_op):
                if self.list_op_dicts[op_id].get(G_OP_INIT_VEH_DIST) is not None:  # specified random distribution
                    init_dist_df = pd.read_csv(os.path.join(self.dir_names[G_DIR_FCTRL], "initial_vehicle_distribution",
                                                            self.scenario_parameters[G_NETWORK_NAME],
                                                            self.list_op_dicts[op_id][G_OP_INIT_VEH_DIST]), index_col=0)
    ...
    def update_sim_state_fleets(self, last_time, next_time, force_update_plan=False):
        ...
        for opid_vid_tuple, veh_obj in sorted(self.sim_vehicles.items(), key=lambda x: self.vehicle_update_order[x[0]]):
            ...
            boarding_requests, alighting_requests, passed_VRL, dict_start_alighting, dict_boarding_no_show_rids, dict_no_show_rid_rq_obj = \
            ...
            if dict_boarding_no_show_rids:      # Ritun change
                # for rq in veh_obj.pax:
                #     rid = rq.get_rid()
                #     print(f"RID in veh {rid}")
                self.operators[op_id].sim_veh_no_show_requests_cleanup(vid, veh_obj, next_time,
                                                                       dict_boarding_no_show_rids.keys())
                veh_obj.no_show_cleanup_happened = True
                for rid, no_show_time_and_pos in dict_boarding_no_show_rids.items():
                    no_show_time, no_show_boarding_pos = no_show_time_and_pos
                    self.demand.record_no_show(rid, vid, op_id, next_time)
                    # self.operators[op_id].acknowledge_no_show(rid, vid, no_show_time)
                # TODO: Record here in demand

            # send update to operator           # Ritun change
    ...
    # def _check_no_show_requests_removal(self, sim_time):
    #     for opid_vid_tuple, veh_obj in self.sim_vehicles.items():
    #         op_id, vid = opid_vid_tuple
    #         no_show_rids_for_veh = list(veh_obj.pending_no_shows.keys())
    #         if veh_obj.pending_no_shows:
    #             try:
    #                 new_plan = self.operators[op_id].sim_veh_no_show_requests_cleanup(vid, veh_obj, sim_time, no_show_rids_for_veh)
    #                 # print(f"pax_info for veh {vid} is {veh_obj.pax_info} at {sim_time} pos 1")
    #
    #                 for rid, no_show_time_and_pos in veh_obj.pending_no_shows.items():
    #                     no_show_time, no_show_pos = no_show_time_and_pos
    #                     self.operators[op_id].acknowledge_no_show(rid, vid, no_show_time)
    #                     self.demand.record_no_show(rid, vid, op_id, no_show_time, no_show_pos)
    #                     del self.operators[op_id].rid_to_assigned_vid[rid]
    #                     self.operators[op_id].rq_dict.pop(rid, None)
    #                 veh_obj.pending_no_shows = {}
    #             except:
    #                 LOG.info(f"Could not remove and assign new plan after no-show removal at {sim_time},"
    #                           f" trying in next step")
        ...
        if chosen_operator is None:  # undecided
            ...
            for i, operator in enumerate(self.operators):
                if i != chosen_operator:
                    ...
                    # TODO: Here collect chosen operator into request obj
    ...
    #             in_vehicle = rq_obj.get_service_vehicle()
    #             request_time = rq_obj.rq_time
    #             if not rq_obj.initial_exp_wait_time_calculated:
    #                 rq_obj.initial_exp_wait_time = self.operators[chosen_operator]._return_expected_pickup_time(rid) - request_time
    #                 rq_obj.initial_exp_wait_time_calculated = True
    #             if in_vehicle is None and chosen_operator is not None:
    ...
    #         chosen_operator = rq_obj.get_chosen_operator()
    #         rq_obj.service_vid_cancel_rq = self.operators[chosen_operator].rid_to_assigned_vid.get(rid)
    #         self.operators[chosen_operator].waiting_user_cancels_request(rid, sim_time)
    #         rq_obj.leave_system_time = sim_time
    #         self.demand.record_user(rid)
    #         del self.demand.rq_db[rid]
    ...
    def _check_request_cancellations_diffusion_model(self, sim_time):           # With ETA update feature copy (This is the latest version)
        """This method builds the interface for traveler models, where users can cancel their booking as per the
         diffusion model.
    ...
    # def _check_waiting_request_cancellations(self, sim_time):     # This based on the two stage binary logit model (Not in use anymore)
    ...
    # def cancel_later_request_removal(self, sim_time, time_step):      # This based on the two stage binary logit model (Not in use anymore)

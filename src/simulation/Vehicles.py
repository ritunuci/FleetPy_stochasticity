...
class SimulationVehicle:
    def __init__(self, operator_id: int, vehicle_id: int, vehicle_data_dir: str, vehicle_type: str, routing_engine: NetworkBase, rq_db: tp.Dict[tp.Any, RequestBase], op_output: str,
                 record_route_flag: bool, replay_flag: bool, scenario_parameters):
        """
        ...
        self.cl_first_start_time = None  # Ritun added: To capture when did the vehicle actually start waiting for the passenger
        # self.no_show_event_for_veh = False  # Ritun added: No show event tracking for vehicle leg     # Ritun change
        self.delayed_plan_assignment = None  # Ritun added: For storing new plan when a locked boarding leg is encountered
        self.no_show_cleanup_assignment_happened = False  # Ritun added: To mark it true in case a no show cleanup assignment happened
        self.pending_no_shows = {}  # Ritun added
    ...
    def start_next_leg(self, simulation_time:int)->tp.Tuple[tp.List[RequestBase], tp.List[RequestBase]]:
        ...
        if self.assigned_route and simulation_time >= self.assigned_route[0].earliest_start_time:
            ...
            if self.cl_first_start_time is None:      # Ritun added: Stores the actual time of a leg start for a no-show
                self.cl_first_start_time = simulation_time
    ...
    def end_current_leg(self, simulation_time:int)->tp.Tuple[tp.List[int], VehicleRouteLeg]:
        ...
        if self.assigned_route and not self.start_next_leg_first:
            ...
            record_dict[G_VR_LEG_WAIT_DELAY] = self.cl_start_time - self.cl_first_start_time  # Ritun added
            # record_dict[G_VR_LEG_NO_SHOW_EVENT_FOR_VEH] = self.no_show_event_for_veh  # Ritun added
            ...
            for rq_obj in ca.rq_dict.get(-1, []):
                try:
                    self.pax.remove(rq_obj)                 # Ritun change no_show

                    # if rq_obj.rid == 990:
                    #     print(f"Removing 990 from vehicle at {simulation_time}")
                    #     for rq_obj in self.pax:
                    #         print(f"pax: {rq_obj.get_rid()}")

                    # if rq_obj.rid == 165:
                    #     print(f"Removing 165 from vehicle at {simulation_time}")
                    #     for rq_obj in self.pax:
                    #         print(f"pax: {rq_obj.get_rid()}")
                except:
            ...
            self.cl_first_start_time = None  # Ritun added: Reset back to None after the leg ends after the no-show wait time
            # self.no_show_event_for_veh = False  # Ritun added: Reset back to False
            # default status and shift to next leg
            self.reset_current_leg()
            self.assigned_route = self.assigned_route[1:]      # Here removes the current leg from the route
    ...
    def assign_vehicle_plan(self, list_route_legs: tp.List[VehicleRouteLeg], sim_time:int, force_ignore_lock:bool = False):
                ...
                if list_route_legs and self.status == VRL_STATES.WAITING and list_route_legs[0].earliest_start_time > sim_time: # dont write multiple waiting legs
                    ...
                    if not self.assigned_route[0].locked:
                        ...
                        if force_ignore_lock and not self.assigned_route[0].status == VRL_STATES.BOARDING:
                            self.end_current_leg(sim_time)
                        elif self.assigned_route[0].no_show_leg:            # Ritun change no_show
                            LOG.info(f"No show leg is being overwritten at {sim_time}")
                        else:
        ...
        if not list_route_legs:
            self.status = VRL_STATES.IDLE

    def _not_shown_yet_rid(self, add_boarding_rids, c_time):
        """This method takes add_boarding_rids as input. It iterates on each boarding_rids and checks if simulation
        time is < rq_actual_show_time. If yes, it adds that rids to the dict to return"""
        _not_shown_yet_rid_dict = {}
        for rid in add_boarding_rids:
            rq_obj = self.rq_db.get(rid)
            # if rq_obj.no_show == True:
                # print(rq_obj.no_show, rid)
            if c_time < rq_obj.rq_actual_show_time:
    ...
    #             for rid in add_boarding_rids:
    #                 # if rid == 990:
    #                 #     print(f"Adding RID 990 at {c_time}")
    ...
    def update_veh_state(self, current_time:float, next_time:float)->tp.Tuple[tp.Dict[tp.Any, tp.Tuple[float, tuple]], tp.Dict[tp.Any, tp.Tuple[float, tuple]], tp.List[VehicleRouteLeg], tp.Dict[tp.Any, tp.Tuple[float, tuple]]]:   # Updated branch after fixing the alighting issue at no-show stop
        """This method updates the current state of a simulation vehicle. This includes moving, boarding etc.
        The method updates the vehicle position, soc. Additionally, it triggers the end and start of VehicleRouteLegs.
        It returns a list of boarding request, alighting requests.
        ...
        if self.start_next_leg_first:
            ...
            if not no_show_dict:
                ...
                for rid in start_alighting_rids:
                    # if rid == 535:
                    #     print(f"rid {535} has started alighting here at {c_time}!!!")
                    dict_start_alighting[rid] = (c_time, self.pos)
                self.start_next_leg_first = False
                if self.no_show_cleanup_happened:
                    if self.status == VRL_STATES.BOARDING:
                        self.cl_remaining_time = 1      # TODO: this can be set to one only if the no-show boarding leg is attempted for the last time, otherwise if it a route leg (becuase there was only one boarding rq which was no-show then the stop will bee dropped) and this should not be set to 1
                    self.no_show_cleanup_happened = False
                    self.no_show_event = False
                    # print(f"Vehicle {self.vid} MOVING-ON POS 1, SIM TIME {c_time}")
                ...
                # print(f"Total no show duration:{c_time - self.cl_first_start_time} where CL duration:{self.cl_duration}")
                LOG.info(f"VEHICLE {self.vid}: TOTAL NO SHOW DURATION:{c_time - self.cl_first_start_time}, where CL duration:{self.cl_duration}, AT SIM TIME {c_time}")
                #TODO: We have to remove the no-show id from pax here because it is being added here with the last call of start_next_leg
                no_show_rids = list(no_show_dict.keys())
                for rid in no_show_rids:
                    dict_boarding_no_show_rids[rid] = (c_time, self.pos)
                    if rid in add_boarding_rids:
                        add_boarding_rids.remove(rid)
                # for rid in add_boarding_rids:
                #     dict_boarding_requests[rid] = (c_time, self.pos)
                # for rid in start_alighting_rids:
        ...
        while remaining_step_time > 0:
            if self.status in G_DRIVING_STATUS:
                ...
                if arrival_in_time_step == -1:
                    ...
                    if self.assigned_route:
                        ...
                        if not no_show_dict:
                            ...
                            for rid in start_alighting_rids:
                                # if rid == 535:
                                #     print(f"rid {535} alighting here at {c_time} Position 1")
                                dict_start_alighting[rid] = (c_time, self.pos)
                        else:
                            # print(f"No-show found 2, IDS: {list(no_show_dict.keys())} for Veh {self.vid} at {c_time}")
                            # no_show_rids = list(no_show_dict.keys())
                            ...
                            for rid in start_alighting_rids:
                                # if rid == 535:
                                #     print(f"rid {535} alighting here at {c_time} Position 2")
                                dict_start_alighting[rid] = (c_time, self.pos)
                            no_show_rq_obj = list(no_show_dict.values())
                            for rq_obj in no_show_rq_obj:
                                if rq_obj in self.pax:
                ...
                if remaining_step_time < self.cl_remaining_time:
                    ...
                    for rid in add_alighting_rq:
                        # if rid == 535:
                        #     print(f"rid {535} has finally alighted here at {c_time}!!!")
                        dict_alighting_requests[rid] = c_time
                    list_passed_VRL.append(passed_VRL)
                    ...
                    if self.assigned_route:
                        ...
                        if not no_show_dict:
                            ...
                            for rid in start_alighting_rids:
                                # if rid == 535:
                                #     print(f"rid {535} alighting here at {c_time} Position 3")
                                dict_start_alighting[rid] = (c_time, self.pos)
                        else:  # This means that a boarding is late
                            no_show_rq_obj = list(no_show_dict.values())
                            # print(f"No-show found 3, IDS: {list(no_show_dict.keys())} for Veh {self.vid} at {c_time}")
                            ...
                            for rid in start_alighting_rids:
                                # if rid == 535:
                                #     print(f"rid {535} alighting here at {c_time} Position 4")
                                dict_start_alighting[rid] = (c_time, self.pos)
                            no_show_rq_obj = list(no_show_dict.values())
                            for rq_obj in no_show_rq_obj:
                                if rq_obj in self.pax:

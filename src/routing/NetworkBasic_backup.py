...
class NetworkBasic(NetworkBase):
    ...
    def loadNetwork(self, network_name_dir, network_dynamics_file_name=None, stochastic_tt=False, scenario_time=None, scenario_parameters=None):         #TODO: UPDATING
        ...
        if stochastic_tt:
            ...
            self.sto_travel_time = scenario_parameters[G_STO_TRAVEL_TIME]
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    # def _set_stoch_edge_tt(self, o_node_index, d_node_index, new_stoch_travel_time):  ## Was written for debugging only
    #     try:
    #         o_node = self.nodes[o_node_index]
    #         d_node = self.nodes[d_node_index]
    #         edge_obj = o_node.edges_to[d_node]
    #     except:
    #         print(f"[ERROR] Missing edge or node: {o_node_index} -> {d_node_index}")
    #         raise

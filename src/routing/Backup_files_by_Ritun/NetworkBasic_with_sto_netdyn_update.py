...
class Node():
    def __init__(self, node_index, is_stop_only, pos_x, pos_y, node_order=None):
        ...
        #
        self.travel_infos_stochastic_from = {}   #Ritun_new
        self.travel_infos_stochastic_to = {}     #Ritun_new
    ...
    def get_travel_stochastic_infos_to(self, other_node_index):   # Ritun_new
        # print("Function is called")
        return self.travel_infos_stochastic_to[other_node_index]

    def get_travel_stochastic_infos_from(self, other_node_index):
        return self.travel_infos_stochastic_from[other_node_index]
...
class NetworkBasic(NetworkBase):
    def __init__(self, network_name_dir, network_dynamics_file_name=None, stochastic_tt_file_name=None, scenario_time=None, scenario_parameters=None):
        # print(f"Creating instance of NetworkBasic: {id(self)}")   #debug
        """
        ...
        self.sto_parameter_dict = {}  # This for stroring the stochastic parameters from the network edge file
        self.loadNetwork(network_name_dir, network_dynamics_file_name=network_dynamics_file_name,
                         stochastic_tt_file_name=stochastic_tt_file_name, scenario_time=scenario_time, scenario_parameters=scenario_parameters)
    ...
    def loadNetwork(self, network_name_dir, network_dynamics_file_name=None, stochastic_tt_file_name=None, scenario_time=None, scenario_parameters=None):
        nodes_f = os.path.join(network_name_dir, "base", "nodes.csv")
        print(f"Loading nodes from {nodes_f} ...")
        nodes_df = pd.read_csv(nodes_f)
        self.nodes = nodes_df.apply(read_node_line, axis=1)               # It is a list of node objects
        edges_f = os.path.join(network_name_dir, "base", "edges.csv")
        time_stamps_f = os.path.join(network_name_dir, "base", "time_stamps.csv")
        print(f"Loading edges from {edges_f} ...")
        edges_df = pd.read_csv(edges_f)

        for _, row in edges_df.iterrows():
            o_node = self.nodes[int(row[G_EDGE_FROM])]
            d_node = self.nodes[int(row[G_EDGE_TO])]
        ...
        # This part is for storing the stochastic parameters in the dictionary from the tt_stochastic.csv file
        # edges_f_sto = os.path.join(network_name_dir, "base", "tt_stochastic.csv")
        edges_f_sto = os.path.join(network_name_dir, "base", stochastic_tt_file_name)
        with open(edges_f_sto) as f:                  #TODO:Ritun
            ...
            for data in csvreader:
                ...
                dist = data["distance"]
                mu = float(sto_tt_parameters[0])
                sigma = float(sto_tt_parameters[1])
                self.sto_parameter_dict[(int(from_node), int(to_node))] = (mu, sigma, dist)
        # Time stamps would be read from the scenario parameter file now
        stamps = scenario_parameters[G_STO_TIME_STAMPS]
...
# ----------------------------------------------------------------------------------------------------------------------
    def update_network(self, simulation_time, update_state = True):
        ...
        :rtype: bool
        """
        # Santi: this method is called in the upper level scripts
        # (e.g., BatchOfferSimulation.py) to update the network at every step
        LOG.debug(f"update network {simulation_time}")
        self.sim_time = simulation_time
        new_tt_flag = False    # Track if new travel times are updated
        if simulation_time in self.time_stamps:
            ...
            for key, value in self.sto_parameter_dict.items():
                ...
                mu, sigma, dist = value
                self._set_stoch_edge_tt(from_node, to_node, self._draw_stochastic_tt(mu, sigma, dist))
            # new_tt_flag = True  # This should not return True to the this function call inside step() method in ImmediateDecisionSimulation.py becasue the operator does not know about the stochastic tt update
        if update_state:
            if self.travel_time_file_folders.get(simulation_time, None) is not None:  # Santi: this is what the operator considers for the routing tasks!
                ...
                new_tt_flag = True  # Travel times were updated from the file and this would be known by the operator
    ...
    def _set_edge_tt(self, o_node_index, d_node_index, new_travel_time):
        ...
        # Santi: question to Flo? Should they draw the value for each link here?
        o_node.travel_infos_to[d_node_index] = (new_tt, dis)
        d_node.travel_infos_from[o_node_index] = (new_tt, dis)

    def _set_stoch_edge_tt(self, o_node_index, d_node_index, new_stoch_travel_time):
        o_node = self.nodes[o_node_index]
        d_node = self.nodes[d_node_index]
        edge_obj = o_node.edges_to[d_node]
        edge_obj.set_tt(new_stoch_travel_time)
        new_tt, dis = edge_obj.get_tt_distance()
        o_node.travel_infos_stochastic_to[d_node_index] = (new_tt, dis)
        d_node.travel_infos_stochastic_from[o_node_index] = (new_tt, dis)

    def _draw_stochastic_tt(self, mu, sigma, dist):      #TODO: We will modify this part to handle travel times only
        mean_tt_normal = float(mu)         # Mean of normally distributed data after taking log of travel times
        tt_std_normal = float(sigma)       # Std of normally distributed data after taking log of travel times
        lognormal_stochastic_tt = math.exp(np.random.normal(mean_tt_normal, tt_std_normal))
        lo_tt = float(dist)/(lognormal_stochastic_tt * 0.44704)     #TODO: We will remove the distance part after we get the data
        return lo_tt
    ...
    def move_along_route(self, route, last_position, time_step, sim_vid_id=None, new_sim_time=None,
        ...
        for i in range(len(route)):
            ...
            tt, td = self.nodes[c_pos[0]].get_travel_stochastic_infos_to(c_pos[1])
            if tt > 86400:

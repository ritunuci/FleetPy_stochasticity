...
class NetworkBasic(NetworkBase):
    def __init__(self, network_name_dir, network_dynamics_file_name=None, scenario_time=None, scenario_parameters=None):
    ...
    def loadNetwork(self, network_name_dir, network_dynamics_file_name=None, scenario_time=None):
        ...
        edges_f = os.path.join(network_name_dir, "base", "edges.csv")     # Ritun: In the edge file we will define the parameters
        ...
        # This part is for storing the stochastic parameters in the dictionary
        with open(edges_f) as f:                  #TODO:Ritun
            csvreader = csv.DictReader(f)
            for data in csvreader:
                self.sto_parameter_dict[(int(data["from_node"]), int(data["to_node"]))] = (data["distance"], data["mean_speed_normal"], data["speed_std_normal"])

        df_time_stamps = pd.read_csv(time_stamps_f)
        self.time_stamps = list(df_time_stamps["time_stamps"])
...
# ----------------------------------------------------------------------------------------------------------------------
    def update_network(self, simulation_time, update_state = True):
        ...
        if simulation_time in self.time_stamps:
            # print(f"...Updating the network at {simulation_time} seconds")
        ...
        if update_state:
            if self.travel_time_file_folders.get(simulation_time, None) is not None:  # Santi: this is what the operator considers for the routing tasks!
                ...
                new_tt_flag = True  # Travel times were updated from the file and this would be know by the operator
    ...
    def _set_stoch_edge_tt(self, o_node_index, d_node_index, new_stoch_travel_time):
        ...
        d_node = self.nodes[d_node_index]
        edge_obj = o_node.edges_to[d_node]
        edge_obj.set_tt(new_stoch_travel_time)          #TODO: Ritun: Don't we need this line here? Because the edge obj does not know about the new travel time yet.
        new_tt, dis = edge_obj.get_tt_distance()
        o_node.travel_infos_stochastic_to[d_node_index] = (new_tt, dis)
        d_node.travel_infos_stochastic_from[o_node_index] = (new_tt, dis)

    def _draw_stochastic_tt(self, mu, sigma, edge_dist):
        dist = float(edge_dist)
        mean_speed_normal = float(mu)   ## Mean of normally distributed data after taking log of speeds
        speed_std_normal = float(sigma)  ## Std of normally distributed data after taking log of speeds
        # lognormal_speed = np.random.lognormal(mean_speed_normal, speed_std_normal)    # Lognormal speed (mph)
        lognormal_speed = math.exp(np.random.normal(mean_speed_normal, speed_std_normal))  # Lognormal speed (mph)
        if lognormal_speed > 70:
            lognormal_speed = 70
        lognormal_stochastic_tt = dist / (lognormal_speed * 0.44704)  # sec
        return lognormal_stochastic_tt

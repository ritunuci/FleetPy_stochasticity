...
class FleetControlBase(metaclass=ABCMeta):
    def __init__(self, op_id: int, operator_attributes: Dict, list_vehicles: List[SimulationVehicle],
        ...
        self.rq_dict_store: Dict[Any, PlanRequest] = {}  # rid_struct -> PlanRequest # Ritun added: Unlike self.rq_dict, this won't be updated
        ...
        self.no_show_veh_plans: Dict[int, VehiclePlan] = {}          # Ritun change
        ...
        self.expected_pickup_time_dict = {}       # Ritun added
    ...
    @abstractmethod
    def receive_status_update(self, vid : int, simulation_time: int, list_finished_VRL:List[VehicleRouteLeg], force_update : bool=True, no_show_rids:list = None):
        ...
        if list_finished_VRL or force_update:
            ...
            # if veh_obj.detector:
            #     for ps in self.veh_plans[veh_obj.vid].list_plan_stops:
            #         print(f"Detector plan stops: {ps} for veh {veh_obj.vid}")

            self.veh_plans[vid].update_plan(veh_obj, simulation_time, self.routing_engine, list_finished_VRL,
                                            rq_dict_prq=self.rq_dict_store)  # Ritun change
        ...
        #         print(f"PUP Detector plan stops: {ps} for veh {veh_obj.vid}")
    ...
    @abstractmethod
    def waiting_user_cancels_request(self, rid: Any, simulation_time: int):
        ...
        Has been overridden in PoolingIRSOnly.py
        """
        pass
    ...
    def acknowledge_no_show(self, rid : Any, vid : int, simulation_time : int):
    ...
    def assign_vehicle_plan(self, veh_obj: SimulationVehicle, vehicle_plan: VehiclePlan, sim_time: int,
        ...
        """
        # if veh_obj.no_show_event_for_veh is True and not vehicle_plan.list_plan_stops:
        #     new_list_vrls = self._build_VRLs(vehicle_plan, veh_obj, sim_time)
        #     veh_obj.assigned_route = []
        # else:                             # Ritun change
    ...
    def _build_VRLs(self, vehicle_plan : VehiclePlan, veh_obj : SimulationVehicle, sim_time : int) -> List[VehicleRouteLeg]:
        ...
        c_pos = veh_obj.pos
        c_time = sim_time
        # if veh_obj.no_show_event_for_veh is True and not vehicle_plan.list_plan_stops:            # Ritun change
        ...
        for pstop in vehicle_plan.list_plan_stops:
            ...
            if len(pstop.get_list_boarding_rids()) > 0 or len(pstop.get_list_alighting_rids()) > 0:
                ...
                for rid in pstop.get_list_boarding_rids():
                    ...
                    real_boarding_duration.append(self.rq_dict[
                                                      rid].get_real_boarding_duration())  # This way, different boarding and alighting times for the same request are considered.

                for rid in pstop.get_list_alighting_rids():
                    boarding_dict[-1].append(self.rq_dict[rid])
                    real_alighting_duration.append(self.rq_dict[
                                                       rid].get_real_alighting_duration())  # This way, different boarding and alighting times for the same request are considered.
                max_stop_duration = max(
                    real_boarding_duration + real_alighting_duration)  # We consider the maximum boarding or alighting time of all request in that stop.
            else:
                boarding = False
            ...
            if status != VRL_STATES.IDLE:
                ...
                if dur is not None:
                    ...
                    stop_duration = 0
                _, c_time = pstop.get_planned_arrival_and_departure_time()
                if status == VRL_STATES.BOARDING:
                    # TODO: Santi
                    list_vrl.append(VehicleRouteLeg(status, pstop.get_pos(), boarding_dict, pstop.get_charging_power(),
                                                    duration=max_stop_duration, earliest_start_time=earliest_start_time,
                                                    earliest_end_time=departure_time,

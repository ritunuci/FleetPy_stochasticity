...
from src.fleetctrl.pooling.immediate.insertion import insertion_with_heuristics, simple_remove, simple_remove_bulk
...
class PoolingInsertionHeuristicOnly(FleetControlBase):
    ...
    def receive_status_update(self, vid, simulation_time, list_finished_VRL, force_update=True, no_show_rids = None):
        ...
        """
        super().receive_status_update(vid, simulation_time, list_finished_VRL, force_update=force_update, no_show_rids=no_show_rids)
    ...
    def waiting_user_cancels_request(self, rid, simulation_time):
        """This method is used to confirm a waiting customer cancellation. This can trigger some database processes.
        :param rid: request id
        :type rid: int
        :param simulation_time: current simulation time
        :type simulation_time: float
    ...
    def sim_veh_no_show_requests_cleanup(self, vid, veh_obj, simulation_time, no_show_rids):
        """This method is used to cleanup the no show requests from a simulation vehicle"""
        # print(f"removing no-show {list(no_show_rids)} at {simulation_time}")
        old_plan = self.veh_plans[vid]
        # print(f"OLD PLAN RIDS: {old_plan.pax_info.keys()} and VID: {vid}")
        # print(f"LEN {len(self.no_show_veh_plans)}")

        # old_plan = self.no_show_veh_plans[vid]     # Ritun change: Last saved backup plan
        ...
        # print(f"Old Plan {old_plan.pax_info}")
        # for ps in old_plan.list_plan_stops:
        #     print(f"Old plan stops: {ps}")


        remove_rids = list(no_show_rids)
        new_plan = simple_remove_bulk(
            veh_obj=veh_obj,
            veh_plan=old_plan,
            remove_rids=remove_rids,
            sim_time=simulation_time,
            routing_engine=self.routing_engine,
            rq_dict=self.rq_dict
        )

        # print(f"New plan: {new_plan}")
        # print(f"New plan stop list: {new_plan.list_plan_stops}")
        # print(f"NEW PLAN RIDS: {new_plan.pax_info.keys()} and VID: {vid}")


        # print(f"Plan after removal {new_plan.pax_info}")
        # for ps in new_plan.list_plan_stops:
        #     print(f"New plan stops: {ps}")


        # if new_plan.pax_info:
        #     self.assign_vehicle_plan(veh_obj, new_plan, simulation_time, force_assign=True)
        #     print(f"no-show assignment success")
        # else:
        #     print(f"{vid} became Idle")
        #     veh_obj.status = VRL_STATES.IDLE
        self.assign_vehicle_plan(veh_obj, new_plan, simulation_time, force_assign=True)
        # print(f"no-show assignment success")

        veh_obj.no_show_plan_backup_flag = False   # Ritun change: set the flag back to false
    ...
    def acknowledge_alighting(self, rid, vid, simulation_time):
        ...
        # del self.veh_plans[vid].pax_info[rid]
    ...
    #             """
    #     LOG.debug(f"acknowledge no-show {rid} from {vid} at {simulation_time}")
    #     print(f"pax_info is now: {self.veh_plans[vid].pax_info} at {simulation_time}")
    #     self.rq_dict[rid].pu_time = None
    #     self.rq_dict[rid].status = G_PRQS_ACC_OFFER
    #     self.rq_dict[rid].service_vehicle = None
    ...
    def assign_vehicle_plan(self, veh_obj, vehicle_plan, sim_time, force_assign=False, assigned_charging_task=None, add_arg=None):
        ...
        for rid in get_assigned_rids_from_vehplan(vehicle_plan):
            ...
            if pax_info:
                self.store_expected_pickup_times(rid, pax_info[0])

    def store_expected_pickup_times(self, rid, exp_pickup_time):  # Ritun added
        self.expected_pickup_time_dict[rid] = exp_pickup_time

    def _return_expected_pickup_time(self, rid):   #Ritun added
        """ This method returns the expected pickup time from the pax_info when called

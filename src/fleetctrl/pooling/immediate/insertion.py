...
def simple_insert(routing_engine : NetworkBase, sim_time : int, veh_obj : SimulationVehicle, orig_veh_plan : VehiclePlan, 
    ...
    if veh_obj.status == VRL_STATES.OUT_OF_SERVICE:
        return
    if veh_obj.no_show_event:    # Ritun change no_show: The vehicle won't be considered while waiting for no-show
        return
    ...
    for i in first_iterator:
        ...
        if not next_o_plan.list_plan_stops[i].is_locked() and not next_o_plan.list_plan_stops[i].is_locked_end() and prq_o_stop_pos == next_o_plan.list_plan_stops[i].get_pos():
            ...
            if new_prq_obj.insertion_with_heterogenous_PUDO_duration != True:
        ...
        for j in second_iterator:
            ...
            if d_stop_pos == next_d_plan.list_plan_stops[j].get_pos() and not next_d_plan.list_plan_stops[j].is_locked_end():
                ...
                new_plan_stop = BoardingPlanStop(d_stop_pos, boarding_dict={-1: [new_rid_struct]}, max_trip_time_dict={new_rid_struct : prq_max_trip_time},
                                                 change_nr_pax=-new_prq_obj.nr_pax, duration=alighting_time)
...
def simple_remove_bulk(veh_obj: SimulationVehicle, veh_plan: VehiclePlan, remove_rids, sim_time: int,
                       routing_engine: NetworkBase, rq_dict: Dict[int, PlanRequest]) -> VehiclePlan:
    """
    Removes a set of request IDs (remove_rids) from the given vehicle plan.
    Ensures pax_info and change_nr_pax are updated correctly.

    :param veh_obj: SimulationVehicle
    :param veh_plan: VehiclePlan
    :param remove_rids: list of rids to remove
    :param sim_time: current simulation time
    :param routing_engine: routing engine for updated VehiclePlan
    :param rq_dict: request dict {rid: PlanRequest}
    :return: new VehiclePlan with updated stops
    """
    # for ps in veh_plan.list_plan_stops:
    #     print(f"change_nr_pax {ps.get_change_nr_pax()}")
    # for ps in veh_plan.list_plan_stops:
    #     print(f"change_nr_pax {ps.is_locked()}")
    # for ps in veh_plan.list_plan_stops:
    #     print(f"change_nr_pax {ps.is_locked_end()}")
    ...
    for ps in veh_plan.list_plan_stops:
        ...
        # Boarding
        for rid in ps.get_list_boarding_rids():
            if rid not in remove_rids:
                new_boarding_dict.setdefault(1, []).append(rid)
                new_earliest_pickup_time_dict[rid] = ept_dict[rid]
                new_latest_pickup_time_dict[rid] = lpt_dict[rid]
            else:
        ...
        if new_boarding_dict or ps.is_locked() or ps.is_locked_end():
            dur, _ = ps.get_duration_and_earliest_departure()
            new_ps = PlanStop(
                ps.get_pos(),
    ...
    for rid in remove_rids:
        try:
            del external_pax_info[rid]
        except:
            pass
    new_veh_plan = VehiclePlan(veh_obj, sim_time, routing_engine, new_plan_list, external_pax_info=external_pax_info)
    # for rid in remove_rids:
    #     if rid in list(new_veh_plan.pax_info.keys()):
    #         print(f"RID {rid} STILL NOT REMOVED")
    missing_rids = set(remove_rids) - set(found_rids)
    if missing_rids:
        LOG.warning(f"Some RIDs were not found in the original plan and couldn't be removed: {missing_rids}")

    # for ps in new_veh_plan.list_plan_stops:
    #     print(f"change_nr_pax {ps.get_change_nr_pax()}")
    # print(len(new_veh_plan.list_plan_stops))

    # print(f"New plan {len(new_veh_plan.list_plan_stops)}")
    # i = 1
    # for ps in new_veh_plan.list_plan_stops:
    #     for rid in ps.get_list_boarding_rids():
    #         print(f"Stop {i} Boarding rids: {rid}")
    #     for rid in ps.get_list_alighting_rids():
    #         print(f"Stop {i} Alighting rids: {rid}")
    #     i+=1
...
def insertion_with_heuristics(sim_time:int, prq: PlanRequest, fleetctrl: FleetControlBase,
                              force_feasible_assignment: bool=True) -> List[Tuple[Any, VehiclePlan, float]]:

...
class PlanRequest:
    ...
    def __init__(self, rq : RequestBase, routing_engine : NetworkBase, min_wait_time : int=0, 
        ...
        """
        # copy from rq
        self.traveler_rq_obj = rq                    # Ritun added
        ...
        if self.insertion_with_heterogenous_PUDO_duration:
            ...
            self.predicted_duration_boarding = self.__durationPUDOprediction_model(self.real_duration_boarding,
                                                                                   self.max_rel_error_black_box_PUDO_duration,
                                                                                   lower_bound_black_box=rq.lower_bound_black_box_PUDO_dur,
                                                                                   upper_bound_black_box=rq.upper_bound_black_box_PUDO_dur)  # Initialized in this step to make sure that the same value is used in every iteration within insertion.py
            self.predicted_duration_alighting = self.__durationPUDOprediction_model(self.real_duration_alighting,
                                                                                    self.max_rel_error_black_box_PUDO_duration,
                                                                                    lower_bound_black_box=rq.lower_bound_black_box_PUDO_dur,
                                                                                    upper_bound_black_box=rq.upper_bound_black_box_PUDO_dur)

            rq.op_predicted_boarding_time = self.predicted_duration_boarding
            rq.op_predicted_alighting_time = self.predicted_duration_alighting
    ...
    def __durationPUDOprediction_model(self, real_duration: int, max_rel_error_black_box_PUDO_duration,
                                       lower_bound_black_box=False, upper_bound_black_box=False) -> int:
        """ This function is used to predict the duration of the PUDO process based on a "black box model.
            ...
            If the predicted value is outside of the range [lower_bound, upper_bound], the value is set to the closest bound.
            "
        ...
        # Santi: TODO: make sure that the upper and lower bounds are also restricted by feasible values (i.e., those used during the drawing)
        if max_rel_error_black_box_PUDO_duration == 0:
            predicted_duration = real_duration
        else:
            lower_bound_unif_interval = real_duration - (max_rel_error_black_box_PUDO_duration / 100) * real_duration
            upper_bound_unif_interval = real_duration + (max_rel_error_black_box_PUDO_duration / 100) * real_duration

            predicted_duration = math.ceil(random.uniform(lower_bound_unif_interval, upper_bound_unif_interval))
            if (lower_bound_black_box != False) and (upper_bound_black_box != False):
                if predicted_duration < lower_bound_black_box:
                    # print("Pred. duration < lower bound")
                    predicted_duration = lower_bound_black_box
                elif predicted_duration > upper_bound_black_box:
                    # print("Pred. duration > upper bound")
                    predicted_duration = upper_bound_black_box

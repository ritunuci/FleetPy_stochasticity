...
class PlanStopBase(metaclass=ABCMeta):
    ...
    @abstractmethod
    def get_latest_start_time(self, pax_infos : dict, rq_dict_prq: dict = None, pax: list = None) -> float:
        """ this function evaluates all time constraints and returns the 
...
class PlanStop(PlanStopBase):
    ...
    def get_latest_start_time(self, pax_infos : dict) -> float:
        ...
        alighting_rids = self.boarding_dict.get(-1, [])                  # Ritun change
        if alighting_rids:
...
class VehiclePlan:
    ...
    def update_plan(self, veh_obj: SimulationVehicle, sim_time: float, routing_engine: NetworkBase, list_passed_VRLs: List[VehicleRouteLeg] = None,
                    keep_time_infeasible: bool = True, rq_dict_prq: dict = None) -> bool:
        ...
        if list_passed_VRLs and self.list_plan_stops:
            for vrl in list_passed_VRLs:
                ...
                if vrl.destination_pos == self.list_plan_stops[0].get_pos():
                    ...
                    if not rid_boarded_at_stop == set(self.list_plan_stops[0].get_list_boarding_rids()):   # Ritun change
        ...
        self.feasible = self.update_tt_and_check_plan(veh_obj, sim_time, routing_engine,
                                                      keep_feasible=keep_time_infeasible, rq_dict_prq=rq_dict_prq)
        return self.feasible
    ...
    def update_tt_and_check_plan(self, veh_obj: SimulationVehicle, sim_time: float, routing_engine: NetworkBase, init_plan_state: dict=None, keep_feasible: bool=False, rq_dict_prq: dict=None):
        ...
        if init_plan_state is not None:
            ...
            for rq in veh_obj.pax:                  # Ritun change
                rid = key_translator.get(rq.get_rid_struct(), rq.get_rid_struct())
                # if veh_obj.no_show_plan_backup_flag:
                self.pax_info[rid] = [rq.pu_time]
                current_pax_dict[rid] = rq
                # if rid == 990:
                #     print(f"990 is in veh {veh_obj.vid}")
            ...
            if c_pos == pstop_pos:
                ...
                for rid in pstop.get_list_boarding_rids():
                    if i == 0 and self.pax_info.get(rid):
                        continue
                    if rid in veh_obj.no_shows_store_for_veh:
                        # print(f"Stop status {pstop.is_locked()}")
                        # print(f"Stop status {pstop.is_locked_end()}")
                        # print(veh_obj.status)
                        continue

                    self.pax_info[rid] = [c_time]
                    c_pax[rid] = 1
                # for rid in pstop.get_list_alighting_rids():
                alighting_rids = list(pstop.get_list_alighting_rids())
                for rid in alighting_rids:
                    if rid in veh_obj.no_shows_store_for_veh:
                        c_nr_pax += 1
                        try:
                            pstop.boarding_dict[-1].remove(rid)  # Ritun change: removing from alighting list
                        except:
                            LOG.warning(f"RID {rid} already removed from alighting boarding_dict")
                        continue
                    try:
                        self.pax_info[rid].append(c_time)
                        # if rid == 990:
                        #     print(f"Appending dropoff time for 990")
                    except:
                        print(f"Alighting RIDS: {pstop.get_list_alighting_rids()} and stop {pstop}")
                        raise
                    try:
                        del c_pax[rid]
                    except KeyError:

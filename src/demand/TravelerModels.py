...
class RequestBase(metaclass=ABCMeta):
    ...
    def __init__(self, rq_row, routing_engine, simulation_time_step, scenario_parameters):
        ...
        self.rq_actual_show_time = None         # Ritun added
        ...
        if rq_row.get(G_RQ_EPT):
            self.earliest_start_time = rq_row.get(G_RQ_EPT)
                ...
                G_AR_MIN_WT):  # TODO RPP : auslagern in ParcelBase + definieren neuer global variable (parcel_min_wait_time)
    ...
    def receive_offer(self, operator_id, operator_offer, simulation_time,
                      sc_parameters=None):  # TODO remove sc_parameters here
    ...
    def create_SubTripRequest(self, subtrip_id, mod_o_node=None, mod_d_node=None, mod_start_time=None,
                              modal_state=None):
...
# -------------------------------------------------------------------------------------------------------------------- #
class BasicRequest_no_show(RequestBase):
    """This is a subclass of BasicRequest class to to take care of the requests who do not/will not show up.
                # Ritun added"""
    type = "BasicRequest_no_show"
    ...
    def __init__(self, rq_row, routing_engine, simulation_time_step, scenario_parameters):
        ...
        self.insertion_with_heterogenous_PUDO_duration = scenario_parameters.get(
            "insertion_with_heterogenous_PUDO_duration",
            False)  # Not used in this file, but necessary to access this variable in insertion.py
        self.max_rel_error_black_box_PUDO_duration = scenario_parameters.get("max_rel_error_black_box_PUDO_duration",
                                                                             False)  # Not used in this file, but necessary to access this variable in insertion.py
        ...
        self.lower_bound_black_box_PUDO_dur = scenario_parameters.get("lower_bound_black_box_PUDO_dur",
                                                                      False)  # Not used in this file, but necessary to access this variable in insertion.py
        self.upper_bound_black_box_PUDO_dur = scenario_parameters.get("upper_bound_black_box_PUDO_dur",
                                                                      False)  # Not used in this file, but necessary to access this variable in insertion.py
        ...
        if self.insertion_with_heterogenous_PUDO_duration:
            if self.max_rel_error_black_box_PUDO_duration is False:
                # error missing value
                raise IOError(
                    f'ERROR: max_rel_error_black_box_PUDO_duration column is not specified in the input demand data '
                    f'(neither in constant_config nor in scenario_config!')
            else:
                try:
                    ...
                    if self.max_rel_error_black_box_PUDO_duration > 100 or self.max_rel_error_black_box_PUDO_duration < 0:
                        raise ValueError(
                            f'ERROR: max_rel_error_black_box_PUDO_duration is '
                            f'{self.max_rel_error_black_box_PUDO_duration}, but it has to be a value between 0 and 100!')
                except:
                    raise TypeError(
                        f'ERROR: max_rel_error_black_box_PUDO_duration is {type(self.max_rel_error_black_box_PUDO_duration)} '
                        f'type, but it has to be a float or integer value between 0 and 100!')

                if self.lower_bound_black_box_PUDO_dur is False:
                    # error missing value
                    raise IOError(
                        f'ERROR: lower_bound_black_box_PUDO_dur column is not specified in the input demand data '
                        f'(neither in constant_config nor in scenario_config!')
                if self.upper_bound_black_box_PUDO_dur is False:
                    # error missing value
                    raise IOError(
                        f'ERROR: upper_bound_black_box_PUDO_dur column is not specified in the input demand data '
                        f'(neither in constant_config nor in scenario_config!')
                ...
                try:
                    ...
                    raise TypeError(
                        f'ERROR: lower_bound_black_box_PUDO_dur is {type(self.lower_bound_black_box_PUDO_dur)} '
                        f'and upper_bound_black_box_PUDO_dur is {type(self.upper_bound_black_box_PUDO_dur)} '
                        f'type, but they have to be a float or integer value!')

                if self.lower_bound_black_box_PUDO_dur >= self.upper_bound_black_box_PUDO_dur:
                    raise ValueError(
                        f'ERROR: lower_bound_black_box_PUDO_dur is {self.lower_bound_black_box_PUDO_dur}, '
                        f'but it has to be smaller than upper_bound_black_box_PUDO_dur {self.upper_bound_black_box_PUDO_dur}!')

                if self.lower_bound_black_box_PUDO_dur < 0:
                    raise ValueError(
                        f'ERROR: lower_bound_black_box_PUDO_dur is {self.lower_bound_black_box_PUDO_dur}, '
                        f'but it has to be a positive value!')

    def no_show_processing(self):
        if self.no_show:
            self.rq_actual_show_time = self.rq_time + LARGE_INT
        else:
            self.rq_actual_show_time = self.rq_time
...
class MasterRandomChoiceRequest(RequestBase):
    ...
    def choose_offer(self, scenario_parameters, simulation_time):
        ...
        LOG.debug(
            f"{self.get_rid_struct()} chooses offer {choice} from options {list_options} | offers {offer_str(self.offer)}")
        return choice

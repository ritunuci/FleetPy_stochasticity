...
calibration_study_folder_name = "calibration_study_final_sc_3"
...
N_BATCH = 55                 # number of BO trials after Sobol warm start #75
TOTAL_TRIALS = N_TRAIN_SOBOL + N_BATCH
BATCH_SIZE = 1               # keep sequential generation/evaluation

USE_SAASBO = False           # Whether to use SAASBO or vanilla BOTORCH_MODULAR for the BO phase.
EXIT_AFTER_SOBOL = True     # If True, run Sobol trials then exit before BO. Set False to run BO.
...
param_bounds = {
    "op_max_wait_time": (1500.0, 3000.0),
    "op_detour_time_factor": (20.0, 100.0),
    "max_rel_error_PUDO_duration": (10.0, 70.0),
    "cancel_wait_time_bound_lower": (900.0, 1400.0),
    "max_cancel_decision_time": (60.0, 720.0),
    "random_term_variance": (2.0, 5.0),
    "decision_threshold": (50.0, 150.0),
    "no_show_wait_time": (20.0, 180.0),
    "CV_group_1_peak": (0.05, 0.5),
    "CV_group_1_offpeak": (0.01, 0.2),
    "CV_group_2_peak": (0.05, 0.6),
    "CV_group_2_offpeak": (0.01, 0.25),
    "CV_group_3_peak": (0.05, 0.9),
...
def read_FleetPy_replications_outputs_and_build_kpi_vec(pooled_tt_mean_dict, file_name):
    ...
    for rs in RANDOM_SEEDS_FOR_FLEETPY:
        ...
        df_op_stats = pd.read_csv(fleetpy_replication_output_path / f"example_depot_time_pool_irsonly_rs_{rs}" / "2-0_op-stats.csv")
        ...
        kpi_vec = np.array([
            ...
            float((df_user_stats["diffusion_cancelled"].sum()/len(df_user_stats["request_id"]))*100),        # % of total demand cancelled
            float((df_user_stats["diffusion_decision_time"] - df_user_stats["rq_time"]).median()),        # average decision time for cancellations (i.e., cancellation duration)
            float((df_op_stats["end_time"] - df_op_stats["first_attempt_time"]).median()),        #TODO: decide wheatther to take mean or median
        ], dtype=float)
...
def compute_day_wise_standardized_mu_sigma_from_real(day):
    ...
    sigma = np.nanstd(Y, axis=0, ddof=1)  # sample std with ddof=1      #TODO: implement to avoid nan cells in the KPI matrix.
...
def update_csv_with_train_x(values):
    ...
    input_csv_df.loc["max_rel_error_black_box_PUDO_duration", "Parameter_Value"] = float(pudo_dur)
...
def rebuild_ax_client_for_bo():
    ...
    bo_generator = Generators.SAASBO if USE_SAASBO else Generators.BOTORCH_MODULAR
    gs_bo_only = GenerationStrategy(
        steps=[
            GenerationStep(
                generator=bo_generator,
                num_trials=-1,
                max_parallelism=1,
            ),
        ]
...
if ax_client_checkpoint_path.exists():
    ...
    if USE_SAASBO and not hasattr(Generators, "SAASBO"):
        raise RuntimeError(
            "This Ax version does not support Generators.SAASBO. "
            "Either upgrade ax-platform or use a compatible Ax version."
        )
...
loop_end = N_TRAIN_SOBOL if EXIT_AFTER_SOBOL else TOTAL_TRIALS
for i in range(start_trial_number, loop_end + 1):
    ...
    if i <= N_TRAIN_SOBOL:
        ...
        stage_name = "SAASBO optimization" if USE_SAASBO else "Bayesian optimization"
...
if EXIT_AFTER_SOBOL:
    if sobol_stds is None:
        compute_and_save_sobol_stds()
    logger.info(
        "EXIT_AFTER_SOBOL=True: All Sobol trials complete. sobol_stds saved. "
        "Set EXIT_AFTER_SOBOL=False and USE_SAASBO=True/False to resume with BO."
    )
    print(
        f"{Fore.YELLOW}"
        "EXIT_AFTER_SOBOL=True: All Sobol trials complete. sobol_stds saved. "
